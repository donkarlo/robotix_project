...
from robotix.mind.cognition.process.kind.memory.composite.interaction.binary.segregation.segregator.segregator import Segregator
from robotix.trace.group.decorator.storaged import Storaged
...
from robotix.trace.group.kind.core.kinds import Kinds as TraceGroupKinds
...
class RosBagYamlMessageSegragator(Segregator, TraceAddValueSubscriber, GroupRamValuesAdditionFinishedSubscriber):
    ...
    def __init__(self, source_memory_component: MemoryComponent, slc: slice):
        ...
        # umvyf stand for UniformatedMultiValuedYamlFile storage
        self._current_uni_formted_multi_value_storage:UniformatedMultiValuedYamlFile = self._source_memory_component.get_trace_group().get_storage()
    ...
    def do_when_a_new_value_is_added_to_ram(self, value: Any) -> None:
        ...
        if isinstance(value, Dic):
            ...
            if new_trace.get_kind().get_name() not in self._trace_groups_dic.get_keys():
                ...
                new_trace_group = TraceGroup.init_from_traces_and_kind_and_name(None, new_trace_group_kind,
                                                                                trace_group_name)
