...
from robotix.ros.bag.topic_stat import TopicStat
...
class TopicFrequencyFinder:
    ...
    Read either:
      ...
      - a .bag file (query 'rosbag info --yaml' and compute avg freq as count/duration).
            ...
            for raw in fp:
                ...
                if in_header and in_stamp:
                    ...
                    if have_secs and have_nsecs:
                        ...
                        # dt accounting (only to know how many valid pairs exist)
                        prev = s["prev_ts"]
                        if prev is not None:
    ...
    def _stats_from_bag_cli(self) -> Dict[str, TopicStat]:
        ...
        try:
            ...
            raise RuntimeError(f"rosbag info failed: {e.stderr.strip()}") from e
    ...
    def _parse_duration_seconds(info_yaml: str) -> Optional[float]:
        """
        Very small YAML-ish parser for the 'duration:' field line in rosbag info --yaml.
    ...
    def _parse_topics_counts(info_yaml: str) -> List[Tuple[str, int]]:
        """
        Parse the 'topics:' section of rosbag info --yaml to get (topic_name, messages_count).
        """
...
if __name__ == "__main__":
    ...
    # Example 2: .bag (fallback via 'rosbag info --yaml', gives avg freq as count/duration)
