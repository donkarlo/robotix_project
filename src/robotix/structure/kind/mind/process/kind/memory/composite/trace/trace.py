...
class Trace(TraceInterface):
    """
    - trace is the connection between nervous system and mind. on one hand it receives potential activity fieelds and on theother hand it converts it to a nested pair_set point such as a dictionary or a composite pattern
    - This is a lost in time memory cell
    An role or a sensor read happens
    - the smalleast pair_set part of the momery being generated by spikes sample_population field
    - this is the boundry between body brain and mind mind
    - spikes can not be decorated or be composite or decorated
    - trace covers from spike activit field to structed pair_set
    ...
    def __init__(self, population_activity_field: Optional[PopulationActivityField], name:Optional[str]):
        ...
        # it is just a point, a pair_set point, without a sample_population it has no meaning
        self._formatted_data: Any = None

        # is it a command? a lidar sensor? a initial_mission? This must be guessed from formatted pair_set
        self._kind: Optional[Any] = None
        # is what is given from sample_population
        self._name = name
    ...
    def set_name(self, name: Optional[str]):
        """
        This can be set from a sample_population such as a trace group
        Args:
            name:
