...
class Composite(MemoryComponent, BaseComposite):
    def __init__(self, internal_trace_group:TraceGroup, name:str):
        ...
        Args:
            internal_trace_group: can be None to only host the link (child) to the next leaf (trace group here) or composite (only link/child or a trace group)
            name:
        """
    ...
    def create_directory_structure(self, root_path:Path, leaf_file_extension: str) -> None:
        ...
        For each Goal: create a file named `leaf.get_name() + leaf_file_extension` inside its parent directory.
