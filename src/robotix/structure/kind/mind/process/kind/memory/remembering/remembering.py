...
class Remembering:
    ...
    The role of remebering by an evidence, episodic or sequence of observer or just will
    - remembering is the process of bringing (meaningful) group from long-term memory
    - remebering is always to reduce the suprise as much as possible. we remeber to make our prediction system better
    """
    def __init__(self, order):
        """
        It is not necessary to set the group os_file as it is already introduced in Memorizing class and Remebering and Memorizing class are bounded in Working class
        """
        self._levels = None
